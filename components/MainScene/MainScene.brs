...
sub showdialog()
    ...
    keyboarddialog.buttons=["OK", "Default", "Save"]
...
sub onKeyPress()
    if m.top.dialog.buttonSelected = 0 ' OK
        loadList(m.top.dialog.text)
    else if m.top.dialog.buttonSelected = 1 ' Default
        m.top.dialog.text = m.global.feedurl
